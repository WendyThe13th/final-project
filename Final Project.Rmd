...
We define two kinds of situations as work faults: (1) the reaction time is negative, which could be caused by falsely recorded; (2)the reaction time is more than one day, which means the company failed to notify within the same day the delay or breakdown happened. There are 62 work faults in total, which is shown by the table above.
```{r}
work_fault_count <- 
  nyc_bus_reaction %>%
  select(company_name, occur_time, inform_time, reaction_time, is_same_time) %>%
...
Then we count the total number of work faults for each company and the percantage by dividing it with the total number of delay for each company. We can see from the table above that generally the ratio of work faults is quite small. The company "ic bu" has the largest ratio of work faults, followed by "grand" and "acme".
...
#make the final tibble with results of prediction probability of 10 logistics regression
map_df(1:10,~logistic_probs(.)) 
